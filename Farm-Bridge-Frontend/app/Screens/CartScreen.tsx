...
const CartScreen = () => {
  ...
  const fetchCart = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${BASE_URI}/cart/get-cart`, {
  ...
  const addToCart = async (productId: String) => {
    try {
      const token = await getToken();
      const response = await axios.post(`${BASE_URI}/cart/add-to-cart`, { productId, quantity: 1 }, {
        headers: { 'Authorization': token ? `Bearer ${token}` : '' }
      });
      setCart(response.data);
  ...
  const updateQuantity = async (cartId: String, quantity: number) => {
    ...
    try {
      const token = await getToken();
      const response = await axios.put(`${BASE_URI}/cart/update-cart/${cartId}`, { quantity }, {
        headers: { 'Authorization': token ? `Bearer ${token}` : '' }
      });
      setCart(response.data);
    } catch (error) {
  ...
  const removeItem = async (cartId: String) => {
    try {
      const token = await getToken();
      const response = await axios.delete(`${BASE_URI}/cart/delete-cart/${cartId}`, {
        headers: { 'Authorization': token ? `Bearer ${token}` : '' }
      });
      setCart(response.data);
    } catch (error) {
